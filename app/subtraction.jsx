...
// Function to generate random subtraction questions under 20 with positive answers
const generateQuestions = (num) => {
  const qs = [];
  for (let i = 0; i < num; i++) {
    const a = Math.floor(Math.random() * 20); // 0 to 19
    const b = Math.floor(Math.random() * (a + 1)); // 0 to a, ensures non-negative answer
...
const Subtraction = () => {
  ...
  // On mount, generate random questions
  useEffect(() => {
    setQuestions(generateQuestions(NUM_QUESTIONS));
  }, []);

  // Generate options for current question once mode & currentQ is set
  useEffect(() => {
  ...
  const handleShareOnTwitter = async () => {
    const tweet = `I scored ${score} out of ${questions.length} on the Subtraction Quiz! ${
  ...
  if (!mode) {
    return (
      <View style={styles.container}>
        <Text style={styles.title}>Start Subtraction Quiz</Text>
        <Pressable style={styles.button} onPress={() => setMode('normal')}>
          <Text style={styles.buttonText}>Normal Mode</Text>
        </Pressable>
        <Pressable style={[styles.button, { backgroundColor: '#3d1452' }]} onPress={() => setMode('timed')}>
  ...
  if (questions.length === 0) {
    // Wait until questions are generated
    return (
      <View style={styles.container}>
